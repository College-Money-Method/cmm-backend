...
def archive_original(job_id: str, video: Path, transcript: Path | None) -> str:
...
def archived_original_exists(prefix: str) -> bool:
    """True when the archived mp4 is still in the bucket (i.e. not yet expired)."""
    if not (prefix and settings.s3_bucket_name):
        return False
    try:
        s3_client().head_object(
            Bucket=settings.s3_bucket_name, Key=f"{prefix}{VIDEO_FILENAME}"
        )
        return True
