...
`/embed/calculators/student-borrowing-8-percent?tl=es`, which is also how you
check your work.
...
visibility, which is exactly what makes the pattern work. The `borrow-ui`
...
Two entries, the singular and plural of a year count. Languages that inflect
differently need the whole phrase, which is why the number is a token inside it
rather than concatenated in script.

### Checking it

Open `/embed/calculators/student-borrowing-8-percent?tl=es` and read it.
...
4. Keep both scripts, with their `id`s. `borrow-formula` is run on its own by the
   test harness; renaming it hides the calculator from the checks.
5. Do not replace a `data-out` label with typed-in text, however static it looks
   (see §2).
6. `data-locked`, `data-over` and `data-total` are styling hooks the script sets.
   Restyle them; do not set them by hand.
7. Run the Checks button. 11/11 before saving a published row.
